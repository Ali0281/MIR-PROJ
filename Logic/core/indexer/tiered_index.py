    ...
    def convert_to_tiered_index(
        ...
        for k, v in current_index.items():
            for doc_id, df in v.items():
                if df >= first_tier_threshold:
                    if k not in first_tier : first_tier[k] = {doc_id : df}
                    else : first_tier[k][doc_id] = df
                elif df >= second_tier_threshold:
                    if k not in second_tier:
                        second_tier[k] = {doc_id: df}
                    else:
                        second_tier[k][doc_id] = df
                else:
                    if k not in third_tier:
                        third_tier[k] = {doc_id: df}
                    else:
                        third_tier[k][doc_id] = df
